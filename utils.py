def test_stationarity(timeseries):
    ...
    movingAverage = timeseries['Close'].rolling(window=12).mean()
    movingSTD = timeseries['Close'].rolling(window=12).std()

    fig = go.Figure()

    fig.add_trace(go.Scatter(x=timeseries.index, y=timeseries['Close'], mode='lines', name='Original',  line=dict(color='rgba(30, 144, 255, 0.8)', width=2)))
